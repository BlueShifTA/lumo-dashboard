...
from lumo_dashboard.core.config import (
    RuntimeConfig,
    RuntimeConfigUpdate,
    get_config,
    update_config,
...
@router.post("/ports")
def set_ports(req: PortConfig) -> RuntimeConfig:
    if req.leader_port == req.follower_port:
        from fastapi import HTTPException

        raise HTTPException(
            status_code=400, detail="Leader and follower ports must differ"
        )
    update_config(
        RuntimeConfigUpdate(
            leader_port=req.leader_port, follower_port=req.follower_port
        )
    )
